    ...
    <?php
        ...
        try {
            ...
            if(strlen($login) <= 50 && strlen($senha) <= 50) {
                require_once('db.php');
                $sqlHash = "SELECT * FROM funcionarios WHERE fun_nome = '$login' || fun_email = '$login' && fun_senha = '$senha'";
                $respHash = $connection->query($sqlHash);
                if(mysqli_num_rows($respHash) > 0) {
                    while($dataHash = mysqli_fetch_assoc($respHash)) {
                        $hash = $dataHash['fun_senha'];
                    ...
                    echo "<div class='error text-white m-3 text-center text-white'><i class='bx bxs-message-alt-error'></i>Falha ao logar, ainda não tem um login para sua conta de funcionário? Consulte outro funcionário que já possua o login.</div>";
                }
            }
        } catch (Exception $e) {
